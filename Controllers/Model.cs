...
abstract class Model
{
    ...
    void Init()
    {
        wave = new bool[MX * MY][]; // 出力サイズ分用意
        compatible = new int[wave.Length][][]; // waveの大きさ分用意 conpatible : 互換性がある
        ...
        distribution = new double[T]; // distribution : 分布
    ...
    {
        // waveが設定できていなければ、Init()関数を呼び出す
        if (wave == null) Init();

        Clear();
        // seedもランダム関数で決めているため、2重ランダム
    ...
    //public abstract Image<Bgra32> Save();
    public abstract void Save(string filename, db_OutputsEntities db);
